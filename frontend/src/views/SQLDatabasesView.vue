<template>
  <div :class="['p-8 min-h-screen', bgContainer]">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 :class="['text-3xl font-bold', textPrimary]">SQL Databases</h1>
        <p :class="['text-sm mt-1', textSecondary]">Azure SQL Server enumeration</p>
      </div>
      <div class="flex items-center space-x-3">
        <!-- Subscription Selector -->
        <select 
          v-model="selectedSubscription" 
          @change="loadSQLServers"
          :class="[
    ...
    <!-- Error Alert -->
      ...
      <div v-if="!selectedSubscription" class="text-center py-12">
        ...
        <p :class="[textPlaceholder]">Choose a subscription from the dropdown to view SQL Servers</p>
      </div>

      <!-- SQL Servers Table -->
      <div v-else-if="sqlServers.length > 0" :class="[bgPrimary, borderPrimary, 'rounded-lg border overflow-hidden']">
        <table class="w-full">
          <thead :class="[bgSecondary]">
            <tr>
              <th :class="['px-6 py-3 text-left text-xs font-medium uppercase tracking-wider', textTertiary]">Name</th>
              <th :class="['px-6 py-3 text-left text-xs font-medium uppercase tracking-wider', textTertiary]">FQDN</th>
              <th :class="['px-6 py-3 text-left text-xs font-medium uppercase tracking-wider', textTertiary]">Version</th>
          ...
          <tbody :class="[dividePrimary, 'divide-y']">
            <tr v-for="server in sqlServers" :key="server.id" :class="[isDark ? 'hover:bg-gray-750' : 'hover:bg-gray-100', 'transition-colors']">
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <div class="flex-shrink-0 h-8 w-8">
                    <svg class="h-8 w-8" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M0 3.5C0 1.567 1.567 0 3.5 0h11C16.433 0 18 1.567 18 3.5v11c0 1.933-1.567 3.5-3.5 3.5h-11C1.567 18 0 16.433 0 14.5v-11z" fill="url(#sqlIconGradient)"/>
                      <defs>
                        <linearGradient id="sqlIconGradient" x1="9" y1="0" x2="9" y2="18" gradientUnits="userSpaceOnUse">
                          <stop stop-color="#00BCF2"/>
                          <stop offset="1" stop-color="#0078D4"/>
                        </linearGradient>
                      </defs>
                      <path d="M4 6h10v2H4V6zm0 3h10v2H4V9zm0 3h7v2H4v-2z" fill="white"/>
                    </svg>
                  </div>
                  <div class="ml-3">
                    <div :class="['text-sm font-medium', textPrimary]">{{ server.name }}</div>
                    <div :class="['text-xs', textSecondary]">{{ server.id }}</div>
                  </div>
                </div>
              </td>
              <td :class="['px-6 py-4 text-sm', textTertiary]">{{ server.fullyQualifiedDomainName }}</td>
              <td :class="['px-6 py-4 whitespace-nowrap text-sm', textTertiary]">{{ server.version }}</td>
              <td :class="['px-6 py-4 whitespace-nowrap text-sm', textTertiary]">{{ server.location }}</td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span :class="[isDark ? 'bg-green-900 text-green-300' : 'bg-green-200 text-green-800', 'px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full']">
                  {{ server.state }}
                </span>
              </td>
            </tr>
      ...
      <div v-else-if="!loading && selectedSubscription" :class="['text-center py-12 rounded-lg border', bgPrimary, borderPrimary]">
        ...
        <h3 :class="['text-lg font-semibold mb-2', textTertiary]">No SQL Servers</h3>
        <p :class="[textPlaceholder]">No SQL servers found in this subscription</p>
      </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    dividePrimary() {
      return this.isDark ? 'divide-gray-700' : 'divide-gray-300';
    }
  },
  mounted() {
  ...
  methods: {
    ...
    async loadSQLServers() {
      ...
      try {
        const response = await axios.get(`http://localhost:5000/api/azure/subscriptions/${this.selectedSubscription}/sql`)
        
        if (response.data.success) {
          this.sqlServers = response.data.sql_servers
        } else {
          this.error = response.data.error || 'Failed to load SQL servers'
        }
      } catch (err) {
